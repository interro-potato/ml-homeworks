...
\begin{enumerate}[leftmargin=\labelsep]
  ...
  \item \textbf{Under a MAP assumption, compute $P(Positive | x)$ of each testing observation.}
        ...
        $$
        \begin{aligned}
          &\mu_{y_3, Positive} = 0.84 & & \qquad \mu_{y_3} = \frac{1.2 + 0.8 + \cdots + 1.2 + 0.8}{9} = 0.9 \\
          &\sigma_{y_3, Positive}^2 = 0.063 & & \qquad \sigma_{y_3}^2 = \frac{1}{9 - 1} \sum_{i=1}^{9} (y_{3, i} - \mu)^2 = 0.0475
        \end{aligned}
        $$
...
\begin{enumerate}[leftmargin=\labelsep,resume]
  ...
  \item \textbf{Using \texttt{scipy}, test the hypothesis “$k$NN is statistically superior to Naïve Bayes regarding
        ...
        We opted to utilize it with a \texttt{alternative = 'less'} parameter, since
        ...
        \begin{itemize}
          \item $H_0$: $kNN$'s accuracy is statistically superior to Naïve Bayes'
          \item $H_1$: $kNN$'s accuracy is statistically inferior (or equal) to Naïve Bayes'
        \end{itemize}
        ...
        After performing the test, we obtained a \textit{p-value} of $\approx 0.9987$ and
        a \textit{t-statistic} of $\approx 4.1109$, which leads us to assert that, given
        $\alpha = 0.05$, we must not reject the null hypothesis.
        Moreover, switching the hypotheses around and performing another
        \texttt{ttest\_rel}, this time with an \texttt{alternative = 'greater'} parameter,
        we get a p-value of $1 - 0.9987 = 0.0013$ (and the same t-statistic):
        we can, therefore, reject the null hypothesis this time around, and say that
        $kNN$ will be either statistically superior or equal to Naïve Bayes regarding accuracy.

        Finally, performing a two-sided test (with \texttt{alternative = 'two-sided'}),
        we get a p-value of $\approx 0.0026$, which leads us to reject this new null hypothesis -
        $kNN$ being equal to Naïve Bayes regarding accuracy - and assert that $kNN$ is not
        (again, given $\alpha = 0.05$) statistically equal to Naïve Bayes regarding accuracy.

        After performing all these tests, we can assert that, for the proposed significance level,
        $kNN$ is statistically superior to Naïve Bayes regarding accuracy.
